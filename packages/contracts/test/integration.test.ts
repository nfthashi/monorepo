...
import { TOKEN_URI_LENGTH_LIMIT } from "../config";
  ...
  for (const [, targetNetwork] of Object.entries(networkJsonFile).filter(
    ...
    describe(`${selfNetwork.name} -> ${targetNetwork.name} `, function () {
      it("should work", async function () {
        ...
        await hashi721Bridge.connect(owner).initialize(connext, wrappedHashi721.address, TOKEN_URI_LENGTH_LIMIT);
        ...
        await expect(
          hashi721Bridge.connect(holder).xCall(destination, relayerFee, asset, to, tokenId, isTokenURIIgnored)
        )
